   ...
   @csrf
    <div class="row">
        <div class="form-group col-lg-6">
            ...
            <select name="bank_id" class="form-control" id="bank_id">
                @foreach(config('bank_info.banks') as $bank)
                    <option value="{{ $bank['bin'] }}" @if ($transfergateway->bank_id == $bank['bin']) selected @endif>{{ $bank['name'] }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group col-lg-6">
            ...
            <input type="text" class="form-control" id="account_no" placeholder="Nhập số tài khoản" name="account_no" value="{{$transfergateway->account_no}}">
            <span class="error-message" id="account_no-error"></span>
        </div>

        <div class="form-group col-lg-6">
            <label for="account_name">Tên chủ tài khoản</label>
            <input type="text" class="form-control" id="account_name" placeholder="Nhập tên chủ tài khoản" name="account_name" value="{{$transfergateway->account_name}}">
        ...
        <div class="form-group col-lg-6">
            ...
            <select name="template" class="form-control" id="template">         
                    <option value="compact" @if ($transfergateway->template == "compact") selected @endif>compact</option>
                    <option value="compact2" @if ($transfergateway->template == "compact2") selected @endif>compact2</option>
                    <option value="qr_only" @if ($transfergateway->template == "qr_only") selected @endif>qr_only</option>
                    <option value="print" @if ($transfergateway->template == "print") selected @endif>print</option>
            </select>
        </div>
